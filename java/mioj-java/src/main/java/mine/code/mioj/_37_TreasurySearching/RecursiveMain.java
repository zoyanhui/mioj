...
public class RecursiveMain {
    ...
    private static boolean solution(int n, Map<Integer, List<Integer>> edges) {
       ...
       boolean[] inpath = new boolean[n];
        for (Integer node : edges.keySet()) {
            ...
            if(dfsCheckCircle(node, marked, edges, inpath)){
                return false;
            }
        }
        return true;
    ...
    private static boolean dfsCheckCircle(int n, boolean[] marked, Map<Integer, List<Integer>> edges, boolean[] inpath) {
        marked[n] = true;
        inpath[n] = true;
        List<Integer> links = edges.get(n);
        if(links != null) {
            for (Integer link : links) {
                if (inpath[link]) {
                    return true;
                }
                if (dfsCheckCircle(link, marked, edges, inpath)) {
                    return true;
                }
            }
        }
