 ...
 * false
 *
 * 解法； 迭代清理图中入度为0的节点，当没有入度为0的节点时，说明有环
 * Created by zhouyanhui on 2019/11/16.
 */

import java.util.*;

public class InLinkMain {
    public static void main(String args[]) {
        ...
        while (scan.hasNextLine()) {
            ...
            edges[Integer.valueOf(s[0])][Integer.valueOf(s[1])] = 1;
        }
        boolean answer = solution(n, edges);
        System.out.println(answer);
    }

    private static boolean solution(int n, int[][] edges) {
        boolean[] noInLinkNodes = new boolean[n];
        while(true) {
            boolean hasNoInLink = false;
