...
public class Main {
    ...
    private static int solution(int[] ints) {
        ...
        robbed[0] = ints[0];
        robbed[1] = ints[1];
        int[] notRobbed = new int[ints.length];
        notRobbed[0] = 0;
        notRobbed[1] = ints[0];
        for (int i = 2; i < ints.length; i++) {
            robbed[i] = notRobbed[i-1] + ints[i];
            notRobbed[i] = Math.max(notRobbed[i-1], robbed[i-1]);
        }
        return Math.max(robbed[ints.length-1], notRobbed[ints.length-1]);
    }
