...
public class Main {
    ...
    private static int solution(String line) {
        ...
        int[][] numsPos = new int[201][];
        int[] numCount = new int[201];
        for (int i = 0; i < s.length; i++) {
            int rNum = s[i] + 100;
            int[] poses = numsPos[rNum];
            if(poses == null){
                poses = new int[200];
                numsPos[rNum] = poses;
            }
            poses[numCount[rNum]++] = i;
        }
        int ret = 0;
        for (int i = 0; i < s.length - 1; i++) {
            int first = s[i] + 100;
            for (int j = i + 1; j < s.length; j++) {
                // the first num is nums[i], the second one is nums[j]
                int second = s[j] + 100;
                ret += 1;
                int curPos = j;
                int delta = second - first;
                int next = delta + second;
                ret += calc(curPos, next, delta, numCount, numsPos);
            }
        }
        return ret;
    }

    private static int calc(int curPos, int next, int delta, int[] numCount, int[][] numsPos) {
        if(next < 0 || next >= numsPos.length){
            return 0;
        }
        if(numCount[next] == 0){
            return 0;
        ...
        int count = 0;
        for (int i = 0; i < numCount[next]; i++) {
            int pos = numsPos[next][i];
            if(pos > curPos){
                count+=1;
                count += calc(pos, next + delta, delta, numCount, numsPos);
            }
        }
        return count;
    }
